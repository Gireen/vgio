...
def load(operator, context, filepath='',
    ...
    if True:
        for model in bsp.models:
            ...
            for face_index, face in enumerate(faces):
                ...
                top_left = math.ceil(min_x / 16), math.floor(min_y / 16)
                ...
                try:
                    ...
                    pixels = list(pixels.reshape(length * 4))
                except ValueError as ve:
                    pass

                #lm_debug_image = bpy.data.images.new('lm.001', *size)
                #lm_debug_image.pixels = pixels

                lightmap_uvs = []
                lightmap_offset = -min_x, -min_y
                lightmap_offset = math.ceil(-min_x / 16) * 16, math.ceil(-min_y / 16) * 16
            ...
            lightmap_count = len(lightmap_infos)
            packed_count = len(atlas_offsets)
            if packed_count != lightmap_count:
                print('Failed to pack {} lightmaps'.format(lightmap_count - packed_count))

            # Create lightmap atlas texture
            ...
            for i, lightmap_info in enumerate(lightmap_infos):
                ...
                for j, loop in enumerate(face.loops):
                    if j > len(lightmap_info.uvs):
                        break

                    uv_coord = lightmap_info.uvs[j]
                    uv_coord = numpy.divide(uv_coord, atlas_size)

                    # 16 pixels per luxel
                    uv_coord = numpy.divide(uv_coord, (16, 16))

                    # Magic number here. Only works when atlas size is 32 x 32
    ...
    for ob in [o for o in bpy.data.objects if o.type == 'MESH']:
        ...
        for mi, m in enumerate(ob.data.uv_textures['UVMap'].data):
            face = bm.faces[mi]
        ...
        for mi, m in enumerate(ob.data.uv_textures['LightMap'].data):
            m.image = lightmap_img

        # DEBUG: Show lightmap by default
        ob.data.uv_textures.active_index = 1
